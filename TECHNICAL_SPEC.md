...
| **Telemetry v2 implemented** | Blueprint §8 steps 1-3: zones, damage/flash events, round_features → citable coach facts (2026-09) | Parser emits PlayerHurt (hitgroup = crosshair proxy, utility damage) + PlayerFlashed (real blind durations, team flashes) through GameStateGate; map_zones + shared resolver make all LLM-visible text speak callouts; features_v2 computes flash-assisted openings, blind seconds, trade windows, exec sync, smoke coverage per (round, side, MR12-aware); evidence pack cites them against 7 new bootstrap baselines (compute_pro_baselines stub upgrades them once pro demos flow through). Fine-tuning engine (§8 step 4) deliberately deferred |
| **Parser library tracking** | demoinfocs-golang **v5** (v5.2.0, 2026-09) — v4 is a dead module line | v4.5.1 (final v4 release) still failed 'unable to find existing entity' on a real Sept-2026 demo; CS2 format support now lands only in the /v5 module (different import path, so go get -u never suggests it). Verified: the failing 339MB Anubis demo parses in ~3s on v5. Regression harness: LOCAL_DEMO=<path> go test ./parser/ -run TestParseLocalDemo |
| **psycopg3 alongside psycopg2** | psycopg[binary,pool] added to the serving image (2026-09) | langgraph-checkpoint-postgres imports psycopg (v3), which needs its own bundled libpq in the slim image; without it every coach job died at import. Checkpointer also falls back to MemorySaver instead of failing the run |
| **Personal mode = the uploader** | derive_mode: only team_id selects TEAM_ANALYSIS; linked Steam ID keeps PERSONAL (2026-09) | Finding the uploader in the demo previously forced team mode — backwards. Personal packs now add uploader-scoped you_* facts (opening duels, traded deaths, headshot rate, flash seconds, utility damage) keyed on the Steam ID, with an explicit 'identity' fact when no ID is linked or it isn't in the demo |
| **Sign in through Steam** | Steam OpenID 2.0 handled by Next.js routes /api/steam/login + /api/steam/callback (2026-09) | The FastAPI flow in api/routes/oauth.py could never complete — Steam redirects a bare browser to the callback, which demanded a Clerk bearer token, and it wrote to linked_accounts, which the identity chain never reads. The Next.js callback runs inside the Clerk cookie session, verifies the assertion via check_authentication, and writes unsafeMetadata.steam_id — the one field uploads/coaching read. Manual ID entry kept as fallback |
| **Demo/match split (Option B) + upload dedupe** | demos = shared artifact (file + telemetry, parsed once); matches = per-user analysis (identity, recon, coaching). Executed with a data reset (2026-09) | Ten teammates uploading the same file used to mean 10 uploads, 10 GCS objects, 10 parses. Browser computes a content fingerprint (size + sha256 of first/last MB) before upload; presign attaches a duplicate to the existing demo and skips the upload; parse completion fans coach jobs out to every match of the demo (reports stay per-uploader). Event tables re-keyed to demo_id; Match keeps read-through properties (demo_id_for is the seam); jobs carry kind-dependent demo_id/match_id. Reset: scripts/reset_option_b.py + revision d4e8f1a2b6c3 |
