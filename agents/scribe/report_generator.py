...
2. "team_report": A markdown string focused on the team's structure, rotation coordination, trade-fragging, utility setups, and communication improvements.
   - CRITICAL REFERENCING RULE: Do NOT refer to teams simply as "Counter-Terrorists" and "Terrorists" in general analysis, as both teams play both sides. Instead, refer to them by their team names defined in the `team_rosters` object. Differentiate their plays on different sides of the map (e.g., "Natus Vincere's CT side hold on A site was solid, but during the T side execute...").
