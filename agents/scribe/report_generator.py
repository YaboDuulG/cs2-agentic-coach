...
_FINDING_SCHEMA = types.Schema(
    ...
    required=["claim", "evidence_ids", "severity", "drill", "audience"],
)
...
async def async_generate_reports(
    ...
    # 3. Stage 2 + 3: synthesis, then verification of the cited evidence
    try:
        synthesis = await _synthesize_findings(client, scout_out, evidence_pack, round_summaries)
    except Exception as e:
    ...
    # 4. Stage 4: render the legacy keys from the verified findings
    return _render_legacy_reports(findings, summary)
